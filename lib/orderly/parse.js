...
EOF: '$end',
yy: {},
symbols: ["$accept","$end","orderly_schema","unnamed_entry",";","named_entries","named_entry","unnamed_entries","definition_prefix","property_name","definition_suffix","string_prefix","string_suffix","INTEGER","optional_range","NUMBER","BOOLEAN","NULL","ANY","ARRAY","{","}","optional_additional_marker","[","]","OBJECT","UNION","STRING","optional_perl_regex","optional_enum_values","optional_default_value","optional_requires","optional_optional_marker","optional_extra_properties","csv_property_names",",","`","JSONObject","<",">","?","*","JSONArray","=","JSONValue","JSONNumber","JSONString","PROPERTY","REGEX","STRING_LIT","NUMBER_LIT","JSONNullLiteral","JSONBooleanLiteral","TRUE","FALSE","JSONText","JSONMemberList","JSONMember",":","JSONElementList"],
nonterminals_: {"orderly_schema":2,"named_entries":5,"unnamed_entries":7,"named_entry":6,"unnamed_entry":3,"definition_prefix":8,"string_prefix":11,"string_suffix":12,"definition_suffix":10,"csv_property_names":34,"optional_extra_properties":33,"optional_requires":31,"optional_optional_marker":32,"optional_additional_marker":22,"optional_enum_values":29,"optional_default_value":30,"optional_range":14,"property_name":9,"optional_perl_regex":28,"JSONString":46,"JSONNumber":45,"JSONNullLiteral":51,"JSONBooleanLiteral":52,"JSONText":55,"JSONValue":44,"JSONObject":37,"JSONMember":57,"JSONMemberList":56,"JSONArray":42,"JSONElementList":59},
...
performAction: function anonymous(yytext, yylineno, yy) {
    ...
    switch (arguments[3]) {
      ...
      case 23:
        ...
        break;
      case 24:
        this.yyval = {enums: arguments[4][arguments[4].length - 5 + 1 - 1], defaultv: arguments[4][arguments[4].length - 5 + 2 - 1], requires: arguments[4][arguments[4].length - 5 + 3 - 1], optional: arguments[4][arguments[4].length - 5 + 4 - 1], extras: arguments[4][arguments[4].length - 5 + 5 - 1]};
        break;
      case 25:
    ...
    while (true) {
        ...
        if (typeof action == "undefined" || !action.length || !action[0]) {
            ...
            for (p in table[state]) {
                if (!this.nonterminals_[p] && p !== EOF) {
                    expected.push("'" + p + "'");
                }
            }
            self.log("stack:", JSON.stringify(stack), "symbol:", symbol, "input", this.lexer.upcomingInput());
            throw new Error("Parse error on line " + (yylineno + 1) + ". Expecting: " + expected.join(", ") + "\n" + this.lexer.showPosition());
        }
...
next:function () {
    ...
    if (this._input == this.EOF) {
        ...
        throw new Error("Lexical error on line " + (this.yylineno + 1) + ". Unrecognized text.\n" + this.showPosition());
    }
},
lex:function () {
    var r = this.next();
